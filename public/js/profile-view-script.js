...
// Admin service for fetching student data
class AdminService {
    static async getStudentProfile(userId) {
        const response = await AuthService.makeRequest(`/admin/student/${userId}`);
        // The actual student data is nested under response.data.student
        if (response && response.data && response.data.student) {
            return { success: true, data: response.data.student };
        }
        return response;
    }
}
...
async function loadStudentData(userId) {
    try {
        ...
        const response = await AdminService.getStudentProfile(userId);

        if (response.success) {
            const studentData = response.data;
            console.log('✅ Student data loaded:', studentData);
            
            // Populate all sections with student data
            populateProfileHeader(studentData.user);
            populateStatsGrid(studentData.stats, studentData.testProgress);
            populatePerformanceSections(studentData.testProgress);
            displayMasteryData(studentData.masterySummary);
        } else {
            throw new Error(response.message || 'Failed to load student data');
        }
    } catch (error) {
        console.error('❌ Error loading student data:', error);
        alert(`Could not load student profile. ${error.message}`);
    }
}
...
function populateProfileHeader(user) {
    ...
    // Update category averages - these are part of the stats object now
    if (user.categoryAverages) {
        document.getElementById('profile-math-avg').textContent = `${user.categoryAverages.math || 0}%`;
        document.getElementById('profile-english-avg').textContent = `${user.categoryAverages.english || 0}%`;
    }
    
    // Hide edit profile button for admin view
...
function populatePerformanceSections(testProgress) {
    // Populate SHSAT Scores section
    const shsatProgress = testProgress.shsat;
    if (shsatProgress && shsatProgress.testsCompleted > 0 && shsatProgress.bestScaledScore.total > 0) {
    ...
    const satProgress = testProgress.sat;
    if (satProgress && satProgress.testsCompleted > 0 && satProgress.bestScaledScore.total > 0) {
...
function displayMasteryData(masterySummary) {
    ...
    const masteryCount = masterySummary ? Object.keys(masterySummary).length : 0;
    document.getElementById('mastery-areas-count').textContent = masteryCount;
